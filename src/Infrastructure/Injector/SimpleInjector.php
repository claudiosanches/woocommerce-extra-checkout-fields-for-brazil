...
final class SimpleInjector implements Injector {
	 ...
	 * @return Injector
	 */
	public function bind_argument( $interface_or_class, $argument_name, $value ) {
	 ...
	 * @return Injector
	 */
	public function delegate( $interface_or_class, $callable ) {
	 ...
	 * @return object Instantiated object.
	 */
	private function make_dependency( InjectionChain $injection_chain, $interface_or_class ) {
		$injection_chain = $this->resolve(
			$injection_chain,
			$interface_or_class
		);
	 ...
	 *
	 * @throws FailedToMakeInstance If failed to make an instance.
	 * @param InjectionChain $injection_chain    Injection chain to track
	 *                                           resolutions.
	 * @param string         $interface_or_class Interface or class to resolve.
	 * @return InjectionChain Modified Injection chain.
	 */
	private function resolve( InjectionChain $injection_chain, $interface_or_class ) {
	 ...
	 *                                           class' constructor.
	 */
	private function get_dependencies_for( InjectionChain $injection_chain, ReflectionClass $reflection, $arguments = [] ) {
	 ...
	 *
	 * @throws FailedToMakeInstance If the class could not have arguments resolved.
	...
	private function resolve_argument_by_name(
		...
		if ( \array_key_exists( $class, $this->argument_mappings ) && \array_key_exists( $name, $this->argument_mappings[ $class ] ) ) {
		...
		} catch ( ExceptionThrowable $exception ) { // phpcs:ignore Generic.CodeAnalysis.EmptyStatement.DetectedCatch
	...
	private function has_shared_instance( $class ) {
		return \array_key_exists( $class, $this->shared_instances ) && null !== $this->shared_instances[ $class ];
	}

	/**
	 * Get the shared instance for a given class.
	 *
	 * @throws FailedToMakeInstance If the class could not share instance.
	 ...
	 *
	 * @throws FailedToMakeInstance If the class could not be delegated.
